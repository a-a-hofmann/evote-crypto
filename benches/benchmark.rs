...
use criterion::{AxisScale, BenchmarkGroup, BenchmarkId, Criterion, criterion_group, criterion_main, PlotConfiguration, SamplingMode, Throughput};
...
fn crypto_material() -> (ElGamalParameters, ElGamalPrivateKey, ElGamalPublicKey, Cipher, Cipher) {
    ...
    let public_key = ElGamalPublicKey { h: h.clone(), params: params.clone() };
...
fn prepare_benchmark<'a>(c: &'a mut Criterion, group_name: &str, sampling_mode: SamplingMode, axis_scale: AxisScale) -> BenchmarkGroup<'a, WallTime> {
...
fn bench_homomorphic_sum(c: &mut Criterion) {
    ...
    let mut group = prepare_benchmark(c, "Homomorphic sum", SamplingMode::Auto, AxisScale::Logarithmic);
    group.sampling_mode(SamplingMode::Auto);

    for count in elements_count.iter() {
        let ciphers0: Vec<Cipher> = iter::repeat(ciphertext0.clone()).take((count / 2) as usize).collect::<Vec<_>>();
        let ciphers1: Vec<Cipher> = iter::repeat(ciphertext1.clone()).take((count / 2) as usize).collect::<Vec<_>>();
        ...
        group.bench_with_input(BenchmarkId::from_parameter(count), &ciphers, |b, ciphers| b.iter(|| {
...
fn bench_decrypt_many_votes(c: &mut Criterion) {
    ...
    for count in elements_count.iter() {
        ...
        group.sample_size(10).bench_with_input(BenchmarkId::from_parameter(count), &sum, |b, sum| b.iter(|| {
...
fn bench_votes_decryption(c: &mut Criterion) {
    ...
    let mut group = prepare_benchmark(c, "Homomorphic sum and decryption", SamplingMode::Flat, AxisScale::Logarithmic);
    ...
    for count in n.iter() {
        ...
        group.throughput(Throughput::Elements(*count as u64));
        group.sample_size(10).bench_with_input(BenchmarkId::from_parameter(count), &ciphers, |b, ciphers| b.iter(|| {
            let sum = ElGamal::add_parallel(ciphers.clone(), &params);
